...
function fonts(){
    return gulp.src(path.src.font) // get all fonts
...
function watch(){
    ...
    gulp.watch(path.watch.js, gulp.series(javascript)).on('change',webserver.reload);
    gulp.watch(path.watch.img, gulp.series(images)).on('change',webserver.reload);
...
const build = series(clean, parallel(html,css, javascript,images,fonts));
exports.build = build;
